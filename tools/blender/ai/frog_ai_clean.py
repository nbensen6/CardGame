"""Clean the Meshy-refined (textured) Frog into the shipped hunter AI model.

Same weld/orient/scale/decimate steps `frog_ai_spike.py` proved on the
untextured preview (candidate A), run instead on the textured `refine`
output so the material comes along. Exports straight to
`game/assets/3d/cast/frog_ai.glb` (the `<id>_ai` naming convention
`AI_ART`/`HUNTER_AI_ART` both use) plus the editable `.blend` beside the
other AI sources.

    blender -b --python tools/blender/ai/frog_ai_clean.py -- <src.glb> <out_glb> <out_blend>
"""
import bpy, os, sys
...
REF_H = 1.15          # current frog.glb's height (feet to top) -- keep the accepted scale
TARGET_TRIS = 5200     # current frog.glb is 5136 tris; stay in the same order
