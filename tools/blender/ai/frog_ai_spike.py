"""First-stage spike: clean a Meshy frog preview into a scaled, welded,
decimated static mesh for comparison against the current Python frog.glb.
NOT wired into the live game (no toon shader, no rig, no animation, no
AI_ART/hunter-display entry) -- that's blocked on the still-open
hunter-display-path request to the fixer. This just proves the raw asset
can be cleaned to the right scale and read well in isolation.

    blender -b --python tools/blender/ai/frog_ai_spike.py -- <src.glb> <out_glb> <out_blend>
"""
import bpy, os, sys
...
REF_H = 1.15          # current frog.glb's height (feet to top), matched so scale stays consistent
TARGET_TRIS = 5200     # current frog.glb is 5136 tris; stay in the same order, not 5x worse
