...
- [ ] **3D pivot** (see `design/guide/3d-pivot.md`) — ~~prove the architecture~~ ✅, ~~3D combat feel~~ ✅ `167fb10`, ~~the overworld (walk a hex island to choose your route)~~ ✅ `5bf3550`, ~~one 3D loop via a phase router~~ ✅ `728027d`. Parity closed since 3D became the default: ~~grip timer~~ ✅ `475ebe1`, ~~multi-pick cards~~ ✅ `3132928`, ~~party/relics/gold HUD~~ ✅ `3b5530f`, ~~coach~~ ✅. ~~combat log~~ ✅, ~~every beast its own 3D body~~ ✅ `f0d351f`. **3D combat has parity with 2D.** 3D staging COMPLETE (`views/location_3d.gd`): ~~reward~~ ✅ ~~won/lost~~ ✅ ~~event~~ ✅ ~~campfire~~ ✅ ~~shop~~ ✅ ~~character select~~ ✅. **The whole run is 3D end to end.** ~~2D client deleted~~ ✅ — `views/combat_view.gd` and its scene are gone; 3D is the only client. Open: traversal co-op.
...
**Delete the 2D client?** ANSWERED 2026-08-06 — yes, backup confirmed on GitHub.
Done: `views/combat_view.gd` + `.tscn` + `.uid` deleted, the router's `FALLBACK_2D`
removed, the harness's 2D-only states retired. Recover from `e9464e1` if ever
needed. Nothing open here.

**Two live grip clocks in co-op?** ANSWERED 2026-08-06 — keep them, they just need
tuning. So this is a numbers job, not a design one: `GRIP_SECONDS` (5.0) in
`views/combat_3d.gd`, ledge placement and `weak_point_height` per beast in
`data/bosses.json`. Nick tunes by feel; see `design/notes/tuning-knobs.md`.
