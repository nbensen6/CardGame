...
## Inbox
- [ ] **depth plan** (see `design/depth-plan.md`) — ~~(1) the MAP~~ ✅ `f301e06` (branching routes, fight/elite/rest/treasure/boss nodes, 6 lesser beasts). ~~(2) deck transformation~~ ✅ `18656c7` (campfires: rest/thin/sharpen; upgrades; skippable rewards). ~~(4) rule-changing relics~~ ✅ `c10a366` (26). Map follow-ups: ~~edges~~ ✅, ~~events~~ ✅ `a7ae962`; still open: shop nodes (needs gold). ~~(3) fight variety~~ ✅ `d31c629`, ~~(5) archetype pools~~ ✅ `358bf5d`, ~~(6) ascension ladder~~ ✅. **Depth plan complete.** Balance pass done `692a453` (sim now models human timing/falls; naive 7% / coord 78% / A8 20%). Onboarding done `f30052c`.
- [ ] **art direction** — the last M1 gap and yours to call: commission, learn Inkscape, or style-lock. See `design/depth-plan.md`.
- [ ] shop nodes (needs a gold economy)
