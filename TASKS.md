...
**Delete the 2D client?** Every phase now has a 3D screen, so `views/combat_view.gd`
and `combat_view.tscn` are only reachable as the router's fallback. Deleting them
removes ~1500 lines and ends double-maintenance on every view change. I've left
them in place because it's a one-way call and they're a working reference (and a
plausible base for a phone build). Say the word and they go.
