...
- [ ] **3D pivot** (see `design/3d-pivot.md`) — ~~prove the architecture~~ ✅, ~~3D combat feel~~ ✅ `167fb10`, ~~the overworld (walk a hex island to choose your route)~~ ✅ `5bf3550`, ~~one 3D loop via a phase router~~ ✅ `728027d`. Parity closed since 3D became the default: ~~grip timer~~ ✅ `475ebe1`, ~~multi-pick cards~~ ✅ `3132928`, ~~party/relics/gold HUD~~ ✅ `3b5530f`, ~~coach~~ ✅. ~~combat log~~ ✅, ~~every beast its own 3D body~~ ✅ `f0d351f`. **3D combat has parity with 2D.** 3D staging: ~~reward~~ ✅, ~~won/lost~~ ✅ (`views/location_3d.gd`). Still open: event/campfire/shop staging (the last three the 2D client owns), then retire it; traversal co-op; orbit camera ✅ and Blender pipeline ✅ done.
