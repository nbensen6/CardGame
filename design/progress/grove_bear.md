...
progress file. `design/renders/grove_bear_pass1_34.png` and `_sil.png` show
a fight-arena ring, not the bear. `design/progress/grove_bear_ground.md`
confirms these are the exact files it scored as the ground, "captured with
`look.sh env grove_bear 1`". Same `look.sh`/`look.cmd` output-naming
collision as `gale_serpent.md` describes.

Needs a fresh `look.sh grove_bear 1` (cast) capture — ideally after the
