...
progress file. `design/renders/shifting_idol_pass1_34.png` and `_sil.png`
show a fight-arena ring, not the idol. `design/progress/
shifting_idol_ground.md` confirms these are the exact files it scored as
the ground, "captured with `look.sh env shifting_idol 1`". Same
`look.sh`/`look.cmd` output-naming collision as `gale_serpent.md` describes.

Needs a fresh `look.sh shifting_idol 1` (cast) capture — ideally after the
