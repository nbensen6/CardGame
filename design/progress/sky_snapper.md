...
progress file. `design/renders/sky_snapper_pass1_34.png` and `_sil.png` show
a fight-arena ring, not the beast. `design/progress/sky_snapper_ground.md`
confirms these are the exact files it scored as the ground, "captured with
`look.sh env sky_snapper 1`". Same `look.sh`/`look.cmd` output-naming
collision as `gale_serpent.md` describes.

Needs a fresh `look.sh sky_snapper 1` (cast) capture — ideally after the
